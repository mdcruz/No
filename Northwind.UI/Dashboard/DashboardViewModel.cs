...
namespace Northwind.UI.Dashboard
{
    public class DashboardViewModel : ViewModel
    {
        ...
        {
            IReadOnlyList<Project> allProjects = _projectRepository.GetProjectList();
            IReadOnlyList<Employee> allEmployees = _employeeRepository.GetEmployeeList();
            IReadOnlyList<Department> allDepartments = _departmentRepository.GetDepartmentList();

            NumberOfProjects = allProjects.Count;
            NumberOfEmployeesOnProjects = allEmployees.Count(x => x.Involements.Any());
            EmployeesOnBench = allEmployees.Count(x => !x.Involements.Any() && !allDepartments.Any(y => y.Head == x));

            NumberOfEmployees = allEmployees.Count;
            FullTimers = allEmployees.Count(x => x.IsFullTimer);
            PartTimers = allEmployees.Count(x => !x.IsFullTimer);

            Tuple<int, int> presale = GetProjectsStats(allProjects, ProjectStage.Presale);
            ProjectsPresale = presale.Item1;
            ProjectsPresalePrice = presale.Item2;

            Tuple<int, int> development = GetProjectsStats(allProjects, ProjectStage.Development);
            ProjectsDevelopment = development.Item1;
            ProjectsDevelopmentPrice = development.Item2;

            Tuple<int, int> closed = GetProjectsStats(allProjects, ProjectStage.Closed);
